...
public class GetAllReviewsQueryHandler : IQueryHandler<GetAllReviewsQuery, PaginationResult<ReviewDto>>
{
    ...
    public GetAllReviewsQueryHandler(IReviewRepository reviewRepository, IUserServiceClient userServiceClient, IMapper mapper)
    {
    ...
    public async Task<PaginationResult<ReviewDto>> Handle(GetAllReviewsQuery request, CancellationToken cancellationToken)
    {
        var specification = request.isHidden == null ? null : new ReviewByIsHiddenSpecification(request.isHidden.Value);
